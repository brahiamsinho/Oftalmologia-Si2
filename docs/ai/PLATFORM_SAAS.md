...
- **Shell (sidebar + navbar):** `app/platform/dashboard/layout.tsx` usa `SidebarProvider`, `PlatformSidebar` y `PlatformHeader` — mismo patrón que `app/(dashboard)/layout.tsx` (sidebar 220px / 64px colapsada, overlay en móvil, área principal `bg-gray-50`, header sticky). Navegación **solo plataforma:** Organizaciones → esta ruta; sin enlace al portal de clínicas (`/login`) en sidebar/header (evita percepción de acceso cruzado; el portal de clínica sigue en su URL pública). “Salir” limpia `platform_access_token` y va a `/platform/login`. En **`/platform/login`** se mantiene enlace “Ir al login de clínica” por si alguien entró por error. Componentes: `frontend/src/components/platform/PlatformSidebar.tsx`, `PlatformHeader.tsx`; contexto `SidebarContext.tsx`.
