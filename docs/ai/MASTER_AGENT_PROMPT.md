...
1. **CONTEXTO OMNIPRESENTE (LEE `docs/ai/`):** El sistema es MULTIPLATAFORMA. Incluye un backend Django, una app web frontend (Next.js) y una app MÓVIL nativa (Flutter). Antes de ejecutar o predecir código, absorbe toda esta documentación base, siendo obligatoria la lectura conjunta de `ARCHITECTURE.md` y `MOBILE_ARCHITECTURE.md`. NUNCA programes ignorando uno de estos pilares.
