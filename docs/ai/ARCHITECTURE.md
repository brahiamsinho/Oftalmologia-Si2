...
- **SHARED_APPS (public):** `django_tenants`, `apps.tenant`, **`apps.platform_admin`** (operadores SaaS: `PlatformAdministrator`), `django.contrib.contenttypes`, sessions/messages/staticfiles, `rest_framework`, `rest_framework_simplejwt`, `corsheaders`, `django_filters`, `apps.core`. _(Auth de Django para usuarios de clínica vive en TENANT_APPS; no hay tabla `Usuario` en `public`.)_
