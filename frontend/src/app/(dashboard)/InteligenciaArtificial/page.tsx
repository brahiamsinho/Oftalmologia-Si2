...
const CU24_LEVEL_STYLES: Record<string, { bg: string; border: string; text: string; label: string }> = {
  CRITICO: { bg: 'bg-red-100', border: 'border-red-300', text: 'text-red-800', label: 'Critico' },
  ALTO: { bg: 'bg-orange-100', border: 'border-orange-300', text: 'text-orange-800', label: 'Alto' },
  MEDIO: { bg: 'bg-amber-100', border: 'border-amber-300', text: 'text-amber-800', label: 'Medio' },
  BAJO: { bg: 'bg-yellow-50', border: 'border-yellow-200', text: 'text-yellow-700', label: 'Bajo' },
  INSUFICIENTE: { bg: 'bg-gray-100', border: 'border-gray-300', text: 'text-gray-700', label: 'Info. insuficiente' },
  INDETERMINADO: { bg: 'bg-gray-100', border: 'border-gray-300', text: 'text-gray-700', label: 'No determinable' },
};
...
const STORAGE_KEY = 'ia_conversacion_id';
...
function StatusPill({ interaction }: { interaction: PatientAssistantInteraction }) {
  if (interaction.clasificacion_urgencia) {
    const level = interaction.clasificacion_urgencia.level;
    const style = CU24_LEVEL_STYLES[level] ?? CU24_LEVEL_STYLES.INDETERMINADO;
    const requiresAttention = interaction.clasificacion_urgencia.requires_human_attention;
    return (
      <span
        className={`inline-flex items-center gap-1 rounded-full border ${style.border} ${style.bg} px-2.5 py-1 text-xs font-semibold ${style.text}`}
      >
        <AlertTriangle className="h-3.5 w-3.5" aria-hidden />
        Urgencia: {style.label}
        {requiresAttention && (
          <span className="ml-0.5 rounded-full bg-red-200 px-1.5 text-[10px] font-bold text-red-800">
            ATENCION
          </span>
        )}
      </span>
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {!isUser && row.interaction.clasificacion_urgencia && (
                          <div className="mt-3 space-y-2 rounded-xl border border-red-200 bg-white/70 px-3 py-2.5">
                            <div className="flex items-center gap-2 text-xs font-bold text-red-800">
                              <ShieldAlert className="h-3.5 w-3.5" aria-hidden />
                              Clasificacion CU24: {row.interaction.clasificacion_urgencia.level}
                              <span className="font-normal text-gray-500">
                                ({(row.interaction.clasificacion_urgencia.confidence * 100).toFixed(0)}% confianza)
                              </span>
                            </div>
                            {row.interaction.clasificacion_urgencia.matched_criteria.length > 0 && (
                              <div className="flex flex-wrap gap-1">
                                {row.interaction.clasificacion_urgencia.matched_criteria.map((c) => (
                                  <span
                                    key={c.code}
                                    className="rounded-md border border-red-200 bg-red-50 px-1.5 py-0.5 text-[11px] font-medium text-red-700"
                                  >
                                    {c.label}
                                  </span>
                                ))}
                            ...
                            <p className="text-[12px] leading-5 text-gray-700">
                              {row.interaction.clasificacion_urgencia.orientation}
                            </p>
                            {row.interaction.clasificacion_urgencia.requires_human_attention && (
                              <div className="rounded-lg border border-red-300 bg-red-100 px-2 py-1.5 text-[11px] font-bold text-red-900">
                                Requiere atencion humana inmediata
                              </div>
                            )}
                          </div>
                        )}
                        {!isUser && row.interaction.requiere_clasificacion_urgencia && !row.interaction.clasificacion_urgencia && (
            ...
            <dl className="mt-4 grid grid-cols-2 gap-3">
              ...
              <div className="rounded-lg bg-gray-50 p-3">
                ...
                <dd className="mt-1 text-xl font-bold text-gray-950">
                  {interactions.filter((item) => item.clasificacion_urgencia).length}
                </dd>
              </div>
              <div className="rounded-lg bg-gray-50 p-3">
                <dt className="text-xs font-medium text-gray-500">Criticas</dt>
                <dd className="mt-1 text-xl font-bold text-red-700">
                  {interactions.filter(
                    (item) => item.clasificacion_urgencia?.requires_human_attention,
                  ).length}
                </dd>
              </div>
              <div className="rounded-lg bg-gray-50 p-3">
                <dt className="text-xs font-medium text-gray-500">Nivel maximo</dt>
                <dd className="mt-1 text-xl font-bold text-gray-950">
                  {(() => {
                    const levels = interactions
                      .filter((i) => i.clasificacion_urgencia)
                      .map((i) => i.clasificacion_urgencia!.level);
                    const order = ['CRITICO', 'ALTO', 'MEDIO', 'BAJO'];
                    for (const lvl of order) {
                      if (levels.includes(lvl)) return lvl.charAt(0) + lvl.slice(1).toLowerCase();
                    }
                    return '—';
                  })()}
                </dd>
