...
import {
  ClipboardList, Plus, Search, X, ChevronDown,
  AlertCircle, Loader2, Trash2, Edit2,
  User, Calendar, FileText, Activity,
  AlertTriangle, CheckCircle2, XCircle, Clock,
  FlaskConical, Stethoscope, ShieldCheck, Info,
...
// ── Helpers visuales ────────────────────────────────────────────────────────
const ESTADO_CFG: Record<string, { bg: string; dot: string; icon: React.ElementType; label: string }> = {
  PENDIENTE:  { bg: 'bg-amber-50 text-amber-700 border-amber-200',   dot: 'bg-amber-400',   icon: Clock,         label: 'Pendiente'  },
  EN_PROCESO: { bg: 'bg-blue-50 text-blue-700 border-blue-200',      dot: 'bg-blue-400',    icon: Activity,      label: 'En proceso' },
  APROBADO:   { bg: 'bg-emerald-50 text-emerald-700 border-emerald-200', dot: 'bg-emerald-500', icon: CheckCircle2, label: 'Aprobado'   },
  OBSERVADO:  { bg: 'bg-orange-50 text-orange-700 border-orange-200', dot: 'bg-orange-400',  icon: AlertTriangle, label: 'Observado'  },
  RECHAZADO:  { bg: 'bg-red-50 text-red-700 border-red-200',         dot: 'bg-red-500',     icon: XCircle,       label: 'Rechazado'  },
};
...
function Toggle({
  checked, onChange, label, description, color = 'teal',
}: {
  checked: boolean; onChange: (v: boolean) => void;
  label: string; description?: string; color?: 'teal' | 'emerald' | 'blue';
}) {
  const bg = checked
    ? color === 'emerald' ? 'bg-emerald-500' : color === 'blue' ? 'bg-blue-500' : 'bg-teal-500'
    : 'bg-gray-300';
  return (
    <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 bg-gray-50 cursor-pointer hover:bg-gray-100 transition-colors select-none">
      ...
      {checked && (
        <CheckCircle2 className={`w-4 h-4 ml-auto flex-shrink-0 ${color === 'emerald' ? 'text-emerald-500' : color === 'blue' ? 'text-blue-500' : 'text-teal-500'}`} strokeWidth={2} />
      )}
    </label>
  );
}
...
function ModalPreoperatorio({
  ...
  const evalsFiltradas     = evaluaciones.filter((e) => e.id_paciente === form.id_paciente);
  ...
  function validate() {
    ...
    if (!form.estado_preoperatorio) e.estado_preoperatorio = 'Selecciona el estado.';
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-gray-400">CU13 · Preparación prequirúrgica</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
          ...
          {intentaAprobar && !puedeAprobar && (
            <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl text-[12.5px] text-amber-700">
              ...
              Para cambiar el estado a <strong className="mx-1">Aprobado</strong> debes marcar
              el <strong className="mx-1">checklist completado</strong> y
              la <strong className="mx-1">aptitud para anestesia</strong>.
            </div>
          )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              >
                <div className="relative">
                  <select
                    ...
                    onChange={(e) =>
                      set('id_evaluacion_quirurgica', e.target.value ? Number(e.target.value) : null)
                    }
                    disabled={!form.id_paciente || evalsFiltradas.length === 0}
                  >
                    <option value="">Sin vinculación a evaluación</option>
                    {evalsFiltradas.map((ev) => (
                      <option key={ev.id_evaluacion_quirurgica} value={ev.id_evaluacion_quirurgica}>
                        Eval. #{ev.id_evaluacion_quirurgica} · {new Date(ev.fecha_evaluacion).toLocaleDateString('es-CO')}
              ...
              <Field label="Fecha programada de cirugía">
                <input
                  ...
                  onChange={(e) => set('fecha_programada_cirugia', e.target.value || null)}
                />
              </Field>
            </div>
          </section>
          ...
          <section>
            <h3 className="text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
              <Activity className="w-3.5 h-3.5" />Estado preoperatorio
            </h3>
          ...
          <section>
            <h3 className="text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
              <ClipboardCheck className="w-3.5 h-3.5" />Checklist preoperatorio
            </h3>
            <div className="space-y-3">
              <Toggle
                ...
                color="teal"
              />
              <Field label="Detalle del checklist" hint="Describe los ítems verificados o pendientes.">
                <textarea
          ...
          <section>
            <h3 className="text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
              <FlaskConical className="w-3.5 h-3.5" />Exámenes preoperatorios
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <section>
            <h3 className="text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
              <Syringe className="w-3.5 h-3.5" />Aptitud y observaciones
            </h3>
            <div className="space-y-3">
              <Toggle
          ...
          <div className="flex items-center justify-end gap-3 pt-2 border-t border-gray-100">
            ...
            >
              {saving ? (
                ...
                <><ShieldCheck className="w-4 h-4" />{isEdit ? 'Guardar cambios' : 'Registrar preoperatorio'}</>
              )}
            </button>
          </div>
        </form>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 ${cfg.bg.split(' ').slice(0, 1).join(' ')}`}>
            <ClipboardList className={`w-4 h-4 ${cfg.bg.split(' ').slice(1, 2).join(' ')}`} strokeWidth={2} />
          </div>
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11.5px] text-gray-400 mt-0.5">
              ...
              {po.id_evaluacion_quirurgica && (
                <span className="ml-2 text-violet-500">· Eval. #{po.id_evaluacion_quirurgica}</span>
              )}
            </p>
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              <span className={`inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full border
                ...
                {po.checklist_completado ? 'Checklist ✓' : 'Checklist pendiente'}
              </span>
              <span className={`inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full border
                ${po.apto_anestesia
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-[15px] font-bold text-gray-900">Eliminar preoperatorio</h3>
...
export default function PreoperatorioPage() {
  ...
  const stats = {
    ...
    aprobados: preoperatorios.filter((p) => p.estado_preoperatorio === 'APROBADO').length,
    proceso:   preoperatorios.filter((p) => p.estado_preoperatorio === 'EN_PROCESO').length,
    pendientes: preoperatorios.filter((p) => p.estado_preoperatorio === 'PENDIENTE').length,
    rechazados: preoperatorios.filter((p) => p.estado_preoperatorio === 'RECHAZADO').length,
  };

  const handleSaved = () => { setShowModal(false); setEditTarget(null); loadPreoperatorios(); };
      ...
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[20px] font-bold text-gray-900 leading-tight">Preoperatorio</h1>
              <p className="text-[12px] text-gray-400">CU13 · Gestión de preparación prequirúrgica</p>
            </div>
          </div>
          <button
          ...
          >
            <Plus className="w-4 h-4" />Nuevo preoperatorio
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'Total',      value: stats.total,      icon: FileText,     color: 'bg-teal-50 text-teal-600'       },
            { label: 'Aprobados',  value: stats.aprobados,  icon: CheckCircle2, color: 'bg-emerald-50 text-emerald-600' },
            { label: 'En proceso', value: stats.proceso,    icon: Activity,     color: 'bg-blue-50 text-blue-600'       },
        ...
        {!loading && error && (
          <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-200 rounded-2xl text-red-700">
            ...
            <button onClick={loadPreoperatorios} className="ml-auto text-[12px] underline hover:no-underline">
              Reintentar
            </button>
          </div>
        )}
            ...
            {preoperatorios.length === 0 ? (
              <div className="text-center py-16">
                <div className="w-14 h-14 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <ClipboardList className="w-7 h-7 text-gray-300" strokeWidth={1.5} />
                </div>
                <p className="text-[14px] font-medium text-gray-500">
                  {search || filtroEstado ? 'Sin resultados para este filtro' : 'Aún no hay registros preoperatorios'}
                </p>
                {!search && !filtroEstado && (
