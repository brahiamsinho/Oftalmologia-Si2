    ...
    try {
      ...
      const msg =
        (
          err as {
            response?: {
              data?: { detail?: string; fecha_programada?: string[] };
            };
          }
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="p-3 bg-orange-50 border border-orange-200 rounded-xl">
            <p className="text-[12.5px] text-orange-700 flex items-start gap-2">
              ...
              <strong className="mx-1">Reprogramada</strong> y se registrará en
              la bitácora.
            </p>
...
function ModalCirugia({
  ...
  const [form, setForm] = useState<CirugiaCreate>(() =>
    isEdit
      ? {
          ...
          fecha_real_inicio: cirugia.fecha_real_inicio
            ? cirugia.fecha_real_inicio.slice(0, 16)
            : null,
          fecha_real_fin: cirugia.fecha_real_fin
            ? cirugia.fecha_real_fin.slice(0, 16)
            : null,
  ...
  const preopsFiltrados = preoperatorios.filter(
    (p) => p.id_paciente === form.id_paciente,
  );
  ...
  function validate() {
    ...
    if (!form.fecha_programada)
      e.fecha_programada = "La fecha programada es obligatoria.";
    if (!form.procedimiento?.trim())
      e.procedimiento = "Describe el procedimiento quirúrgico.";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-6">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-indigo-100 rounded-xl flex items-center justify-center">
              <Slice className="w-4 h-4 text-indigo-600" strokeWidth={2} />
            </div>
            <div>
              <h2 className="text-[15px] font-bold text-gray-900">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field
                ...
                hint={
                  !form.id_paciente
                    ? "Primero selecciona un paciente."
                    : preopsFiltrados.length === 0
                      ? "Sin preoperatorios para este paciente."
                      : undefined
                }
              >
                <div className="relative">
                  ...
                  >
                    ...
                    {preopsFiltrados.map((po) => (
                      <option
                        ...
                        value={po.id_preoperatorio}
                      >
                        Preop. #{po.id_preoperatorio} ·{" "}
                        {po.estado_preoperatorio}
                        {po.fecha_programada_cirugia
                          ? ` · ${formatFechaCorta(po.fecha_programada_cirugia)}`
                          : ""}
                      </option>
                    ))}
                  </select>
              ...
              <Field label="Estado" required error={errors.estado_cirugia}>
                <div className="relative">
                  <select
                    className={selectCls(errors.estado_cirugia)}
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <Field
                ...
                error={errors.fecha_real_inicio}
                hint={estadoFinalizada ? "Requerida al finalizar." : undefined}
              >
                <input
                  type="datetime-local"
                  className={inputCls(errors.fecha_real_inicio)}
                  value={form.fecha_real_inicio ?? ""}
                  onChange={(e) =>
                    set("fecha_real_inicio", e.target.value || null)
                  }
                />
              </Field>
              <Field
                label="Fecha real de fin"
                error={errors.fecha_real_fin}
                hint={estadoFinalizada ? "Requerida al finalizar." : undefined}
              >
                <input
                  type="datetime-local"
                  className={inputCls(errors.fecha_real_fin)}
                  value={form.fecha_real_fin ?? ""}
                  onChange={(e) =>
                    set("fecha_real_fin", e.target.value || null)
                  }
                />
              </Field>
            </div>
          ...
          <section>
            ...
            >
              <textarea
                className={textareaCls(errors.procedimiento)}
                rows={3}
          ...
          <section>
            ...
            <div className="space-y-3">
              <Field
                label="Resultado"
                hint="Resumen de resultados y evolución intraoperatoria."
              >
                <textarea
          ...
          <section>
            ...
            <div className="space-y-3">
              ...
              {estadoReprogramada && (
                <Field
                  label="Motivo de reprogramación"
                  error={errors.motivo_reprogramacion}
                >
                  <textarea
                    className={textareaCls(errors.motivo_reprogramacion)}
                    rows={2}
                    placeholder="Razón por la que se reprogramó…"
                    value={form.motivo_reprogramacion as string}
                    onChange={(e) =>
                      set("motivo_reprogramacion", e.target.value)
                    }
                  />
                </Field>
              )}
            ...
            >
              {saving ? (
                ...
                <>
                  ...
                  {isEdit ? "Guardar cambios" : "Registrar cirugía"}
                </>
              )}
            </button>
          </div>
...
function CirugiaCard({
  ...
  const duracionMin = tieneDuracion
    ? Math.round(
        (new Date(cir.fecha_real_fin!).getTime() -
          new Date(cir.fecha_real_inicio!).getTime()) /
          60000,
      )
    : null;
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-4 hover:shadow-md hover:border-gray-300 transition-all group">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0">
          {/* Ícono de estado */}
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 ${cfg.bg.split(" ").slice(0, 1).join(" ")}`}
          >
            <Slice
              className={`w-5 h-5 ${cfg.bg.split(" ").slice(1, 2).join(" ")}`}
              strokeWidth={2}
            />
          </div>
          ...
          <div className="min-w-0 flex-1">
            ...
            {/* Reprogramación */}
            {cir.estado_cirugia === "REPROGRAMADA" &&
              cir.motivo_reprogramacion && (
...
export default function CirugiasPage() {
  ...
  const stats = {
    ...
    programadas: cirugias.filter((c) => c.estado_cirugia === "PROGRAMADA")
      .length,
    enCurso: cirugias.filter((c) => c.estado_cirugia === "EN_CURSO").length,
    finalizadas: cirugias.filter((c) => c.estado_cirugia === "FINALIZADA")
      .length,
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-indigo-600 rounded-2xl flex items-center justify-center shadow-sm">
              <Slice className="w-5 h-5 text-white" strokeWidth={2} />
            </div>
            <div>
            ...
            {cirugias.length === 0 ? (
              <div className="text-center py-16">
                <div className="w-14 h-14 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <Slice className="w-7 h-7 text-gray-300" strokeWidth={1.5} />
                </div>
                ...
                {!search && !filtroEstado && (
                  <p className="text-[12px] text-gray-400 mt-1">
                    Registra la primera con el botón de arriba.
                  </p>
                )}
              </div>
            ) : (
              <div className="space-y-3">
                <p className="text-[12px] text-gray-400 px-1">
                  {total} cirugía{total !== 1 ? "s" : ""} encontrada
                  {total !== 1 ? "s" : ""}
                </p>
                {cirugias.map((cir) => (
                  <CirugiaCard
                    ...
                    cir={cir}
                    pacienteNombre={
                      pacienteMap[cir.id_paciente] ??
                      `Paciente #${cir.id_paciente}`
                    }
                    onEdit={() => {
                      setEditTarget(cir);
                      setShowModal(true);
      ...
      {reprogTarget && (
        <ModalReprogramar
          ...
          pacienteNombre={
            pacienteMap[reprogTarget.id_paciente] ??
            `Paciente #${reprogTarget.id_paciente}`
          }
