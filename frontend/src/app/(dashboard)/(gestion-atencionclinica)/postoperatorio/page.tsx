...
function ModalPostoperatorio({
    ...
    isEdit
      ? {
          ...
          proximo_control: postop.proximo_control
            ? postop.proximo_control.slice(0, 16)
            : null,
  ...
  const cirugiasFiltradas = cirugias.filter(
    (c) => c.id_paciente === form.id_paciente,
  );
  ...
  function validate() {
    ...
    if (!form.fecha_control)
      e.fecha_control = "La fecha del control es obligatoria.";
    if (form.proximo_control && form.fecha_control) {
      if (new Date(form.proximo_control) < new Date(form.fecha_control)) {
        e.proximo_control =
          "El próximo control debe ser posterior al control actual.";
      }
    }
    return e;
  }
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
          ...
          {esComplicado && (
            <div className="flex items-start gap-2 p-3 bg-red-50 border border-red-200 rounded-xl text-[12.5px] text-red-700">
              ...
              Estado <strong className="mx-1">Complicado</strong> — documenta
              las alertas clínicas con detalle para trazabilidad.
            </div>
          )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              >
                <div className="relative">
                  <select
                    ...
                    disabled={
                      !form.id_paciente || cirugiasFiltradas.length === 0
                    }
                  >
              ...
              <Field label="Estado del seguimiento" required>
                <div className="relative">
                  <select
                    className={selectCls()}
                    value={form.estado_postoperatorio}
                    onChange={(e) =>
                      set("estado_postoperatorio", e.target.value)
                    }
                  >
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field
                ...
                required
                error={errors.fecha_control}
              >
                <input
                  type="datetime-local"
                  className={inputCls(errors.fecha_control)}
              ...
              <Field
                ...
                error={errors.proximo_control}
                hint="Debe ser posterior a la fecha del control actual."
              >
                <input
                  type="datetime-local"
                  className={inputCls(errors.proximo_control)}
                  value={form.proximo_control ?? ""}
                  onChange={(e) =>
                    set("proximo_control", e.target.value || null)
                  }
                />
              </Field>
            </div>
          ...
          <section>
            ...
            >
              <textarea
                className={`${textareaCls()} ${form.alertas?.trim() ? "border-amber-300 bg-amber-50/50 focus:ring-amber-400" : ""}`}
                rows={3}
...
function PostopCard({
  ...
  const cfg =
    ESTADO_CFG[po.estado_postoperatorio] ?? ESTADO_CFG["EN_OBSERVACION"];
  ...
  const borderAccent = esComplicado
    ? "border-l-red-500"
    : po.estado_postoperatorio === "ESTABLE"
      ? "border-l-emerald-500"
      : po.estado_postoperatorio === "CERRADO"
        ? "border-l-gray-300"
        : "border-l-blue-500";

  return (
    <div
      className={`bg-white border border-gray-200 border-l-4 ${borderAccent} rounded-2xl p-4 hover:shadow-md transition-all group`}
    >
        ...
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            ...
            {po.proximo_control && !esCerrado && (
              <div
                className={`inline-flex items-center gap-1.5 mt-1.5 px-2.5 py-1 rounded-xl border text-[11.5px] font-medium
                ${
                  dias !== null && dias <= 0
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-[15px] font-bold text-gray-900">
              Eliminar control postoperatorio
            </h3>
...
export default function PostoperatorioPage() {
  ...
  const stats = {
    ...
    estables: postops.filter((p) => p.estado_postoperatorio === "ESTABLE")
      .length,
    observacion: postops.filter(
      (p) => p.estado_postoperatorio === "EN_OBSERVACION",
    ).length,
    complicados: postops.filter((p) => p.estado_postoperatorio === "COMPLICADO")
      .length,
  };
      ...
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        {proximosUrgentes > 0 && (
          <div className="flex items-center gap-3 p-3.5 bg-amber-50 border border-amber-200 rounded-2xl">
            ...
            <p className="text-[13px] text-amber-700">
              <strong>{proximosUrgentes}</strong> control
              {proximosUrgentes !== 1 ? "es" : ""} con próximo control vencido o
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            {
              label: "Estables",
              value: stats.estables,
              icon: CheckCircle2,
              color: "bg-emerald-50 text-emerald-600",
            },
            {
              label: "Observación",
              value: stats.observacion,
              icon: Eye,
              color: "bg-blue-50 text-blue-600",
            },
            {
              label: "Complicados",
              value: stats.complicados,
              icon: AlertTriangle,
            ...
            >
              ...
              <div>
                <p
                  className={`text-[20px] font-bold leading-none ${label === "Complicados" && value > 0 ? "text-red-600" : "text-gray-900"}`}
                >
                  {value}
                </p>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Buscar por paciente, alertas, observaciones…"
              value={search}
        ...
        {filtroFecha && (
          <p className="text-[11.5px] text-gray-400 flex items-center gap-1.5 -mt-2 px-1">
            ...
            Mostrando controles del día{" "}
            {new Date(filtroFecha + "T00:00").toLocaleDateString("es-CO", {
              day: "numeric",
              month: "long",
              year: "numeric",
            })}
        ...
        {loading && (
          <div className="flex items-center justify-center py-16 gap-3 text-gray-400">
            ...
            <span className="text-[13px]">
              Cargando controles postoperatorios…
            </span>
          </div>
        )}
            ...
            {postops.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-[14px] font-medium text-gray-500">
                  {search || filtroEstado || filtroFecha
                    ...
                    : "Aún no hay controles postoperatorios"}
                </p>
                {!search && !filtroEstado && !filtroFecha && (
              ...
              <div className="space-y-3">
                <p className="text-[12px] text-gray-400 px-1">
                  {total} control{total !== 1 ? "es" : ""} encontrado
                  {total !== 1 ? "s" : ""}
                </p>
                {postops.map((po) => (
                  <PostopCard
