...
_PATIENT_ASSISTANT_SYSTEM_PROMPT = """Eres el asistente virtual de una clinica oftalmologica.

Tu unica tarea es reformular una respuesta base que ya fue validada por reglas.

Reglas obligatorias:
1. No agregues diagnósticos, dosis, estudios, medicamentos ni instrucciones nuevas.
2. No cambies el nivel de urgencia ni elimines una derivacion.
3. Conserva el mismo sentido clinico o administrativo de la respuesta base.
4. Responde en español, con tono claro, humano y breve.
5. Si la respuesta base indica urgencia o derivacion, mantenelo sin suavizar.
6. Usa texto plano. No uses markdown ni listas innecesarias.
7. Si no puedes mejorar sin cambiar el contenido, devuelve una version equivalente.
"""
...
class AsistenteVirtualService:
    ...
    @classmethod
    def responder(cls, mensaje: str, history: list[dict[str, object]] | None = None) -> ResultadoAsistenteVirtual:
        ...
        for intencion, terminos, respuesta in cls._INTENCIONES:
            if _contiene(normalizado, terminos):
                resultado = ResultadoAsistenteVirtual(
                    respuesta=respuesta,
                    intencion=intencion,
                    estado=EstadoRespuestaAsistente.RESPONDIDA,
                    metadata={'historial_utilizado': len(historial)},
                )
                if intencion in _INTENCIONES_REFORMULABLES:
                    return cls._reformular_respuesta(resultado, mensaje=texto, history=historial)
                return resultado
    ...
    @staticmethod
    def _reformular_respuesta(
        resultado: ResultadoAsistenteVirtual,
        *,
        mensaje: str,
        history: list[dict[str, object]],
    ) -> ResultadoAsistenteVirtual:
        metadata = dict(resultado.metadata)
        metadata['historial_utilizado'] = len(history)

        try:
            assistant = GeminiChatbotAssistant(system_instruction=_PATIENT_ASSISTANT_SYSTEM_PROMPT)
            prompt = (
                'Consulta original del paciente:\n'
                f'{mensaje.strip()}\n\n'
                'Respuesta base segura:\n'
                f'{resultado.respuesta.strip()}\n\n'
                'Reformula la respuesta base sin cambiar su significado ni agregar datos nuevos.'
            )
            gemini_result = assistant.generate_reply(message=prompt, history=history)
            respuesta = (gemini_result.get('reply') or '').strip()
            if not respuesta:
                raise GeminiChatbotError('Gemini no devolvio una respuesta util.')

            metadata.update(
                {
                    'gemini_usado': True,
                    'gemini_modelo': gemini_result.get('model', ''),
                },
            )
            return replace(resultado, respuesta=respuesta, metadata=metadata)
            ...
            return replace(resultado, metadata=metadata)
